-- =============================================================================
--  0039_avt_purchases_invoice_date
--
--  Purchases window-filter date source ONLY. Everything else in
--  compute_avt_for_audit is BYTE-IDENTICAL to 0038 (window_count1 / v5): auth
--  gate, window-end + window-phase resolution, master_size, actuals/starts
...
--
--  WHY (confirmed against prod 2026-06-12):
--    compute_avt_for_audit sources PURCHASES from v_effective_receipts filtered
--    by er.received_at. But receiving_events is EMPTY (0 rows, all venues), so
--    v_effective_receipts.received_at is ALWAYS NULL → the purchases window
--    filter (er.received_at >= start AND er.received_at < end) matches 0 rows →
--    purchases compute as 0 for every venue on every report.
--    The real purchase data lives in invoice_lines (295k rows) under invoices,
--    whose invoice_date IS populated (57,290 invoices). The purchase_lines CTE
--    already joins `invoices i on i.id = er.invoice_id`, so i.invoice_date is
--    in scope and is the best-available receipt date until receiving_events is
--    populated.
--
--  CHANGE (purchases window filter ONLY — recount/depletion/units/window-phase
--  logic untouched):
--    1. purchase_lines CTE window filter:
--         and er.received_at >= coalesce(v_window_start, '-infinity'::timestamptz)
--         and er.received_at <  v_window_end
--       →
--         and coalesce(er.received_at, i.invoice_date::timestamptz)
--               >= coalesce(v_window_start, '-infinity'::timestamptz)
--         and coalesce(er.received_at, i.invoice_date::timestamptz)
--               <  v_window_end
--       invoice_date is a `date`, cast to timestamptz. A line with NEITHER a
--       received_at NOR an invoice_date is excluded — correct (no date = can't
--       place it in any window). The receipt_uom_unparsed notes subquery, which
--       mirrors this CTE's scope, gets the SAME coalesce so it stays in sync.
--    2. notes.purchases_by_invoice_date = count of distinct purchase lines that
--       matched via the invoice_date fallback (received_at null, invoice_date
--       in-window), so we can see how much of purchases came from the fallback
--       vs real receiving dates. Today this is ~100% since received_at is
--       always null; it drops as receiving_events fills in.
--
...
begin
  ...
  --     i.invoice_date::timestamptz) — receiving_events is empty so
...
comment on function public.compute_avt_for_audit(uuid) is
  'Computes a kount_avt_reports + kount_avt_rows pair for the given audit from in-DB data (kount_entries + kount_recounts overrides, v_effective_receipts, pos_check_items, recipes/maps). v4 units (0037) preserved verbatim. v5 (0038): window end resolves coalesce(count2_closed_at, completed_at, count1_closed_at) so the function can be called at COUNT 1 CLOSE (interim variance for the recount list), raising 22023 only when all three are null; notes.window_phase = count1|count2. v6 (0039): purchases window filter uses coalesce(received_at, invoice_date::timestamptz) because receiving_events is empty so received_at is always NULL; notes.purchases_by_invoice_date tallies lines matched via the invoice_date fallback. Idempotent: re-running replaces the prior computed report for the same audit. Never touches uploaded reports.';
...
-- -----------------------------------------------------------------------------
--
-- (a) Seed the email the auth gate reads via auth.jwt() ->> 'email' so the
--     SECURITY DEFINER function passes the app_users corporate/venue check.
--       select set_config('request.jwt.claims',
--                         '{"email":"apurandare@hwoodgroup.com"}', false);
--
-- (b) Compute for a venue that HAS current invoices, to prove purchases now
--     compute > 0. No submitted "Delilah Miami" audit exists in prod; the
--     venue with the most submitted audits AND invoices is Delilah LA
--     (ops_venue_id 11111111-1111-1111-1111-111111111111, 13,614 invoices).
--     Its most recent submitted audit is below. (If the orchestrator prefers a
--     temp recompute against a different venue, swap the audit id.)
--       select compute_avt_for_audit('0a9edd0d-5623-45e1-91b3-a7658fa3e221');
--       select notes ->> 'window_phase'             as window_phase,
--              notes ->> 'window_start'             as window_start,
--              notes ->> 'window_end'               as window_end,
--              notes ->> 'purchases_by_invoice_date' as via_invoice_date,
--              row_count
--         from kount_avt_reports
--        where audit_id = '0a9edd0d-5623-45e1-91b3-a7658fa3e221'
--          and source = 'computed';
--     Expect purchases_by_invoice_date > 0 (receiving_events empty ⇒ every
--     matched purchase line falls back to invoice_date).
--
-- (c) Total purchases summed across the resulting kount_avt_rows — should now
--     be > 0 where on 0038 it was 0 (received_at always NULL):
--       select sum(r.purchases)               as total_purchases,
--              count(*) filter (where r.purchases > 0) as rows_with_purchases
--         from kount_avt_rows r
--         join kount_avt_reports rep on rep.id = r.report_id
--        where rep.audit_id = '0a9edd0d-5623-45e1-91b3-a7658fa3e221'
--          and rep.source = 'computed';
--
-- Regression: a submitted audit's recount/depletion/units/window numbers are
-- unchanged from 0038 except purchases (and theo, which depends on purchases).
